...
public extension Routable {
    ...
    func simpleRequest(_ method: ServiceMethod,
                       ...
                       encoding: ParameterEncoding = JSONEncoding.default,
                       success: SuccessJSONBlock!,
                       failure: FailureErrorBlock!) -> DataRequest{
        ...
        let request = Alamofire.request(urlString, method: method.urlMethod, parameters: parameters, encoding: encoding, headers: authorizationHeadersIf(authorized)).responseString { response in
            
            QL1("urlString: \(urlString)")
            QL1("Success: \(response.result.isSuccess)")
            QL1("Response String: \(String(describing: response.result.value))")
        }
    ...
    func multipartRequest(_ method: ServiceMethod,
        ...
        let URL = try! URLRequest(url: urlString, method: method.urlMethod, headers: headers)
    ...
    func handleServerResponse(_ response: DataResponse<JSONTopModal>, success: SuccessJSONBlock!, failure: FailureErrorBlock!) {
        ...
        let tmpResult = Utility.validateServerResponse(with: response.response!)
        if tmpResult.0 == true {
            failure(NSError.init(errorMessage: resultValue.message))
        }
