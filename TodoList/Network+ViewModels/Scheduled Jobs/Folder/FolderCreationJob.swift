//
//  FolderCreationJob.swift
//  TodoList
//
//  Created by Usama Jamil on 05/06/2020.
...
class FolderCreationJob: Job {
    
    // Type to know which Job to return in job creator
    static let type = "FolderCreationJob"
    // Param
    ...
    func onRun(callback: JobResult) {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + App.Constants.jobDelay) { // Change `2.0` to the desired number of seconds.
            ...
            self.folderLD.createFolder(param: self.param)
...
extension FolderCreationJob: NetworkDelegate {
