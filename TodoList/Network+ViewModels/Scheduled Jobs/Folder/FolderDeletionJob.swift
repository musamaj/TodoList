//
//  FolderDeletionJob.swift
//  TodoList
//
//  Created by Usama Jamil on 09/06/2020.
...
class FolderDeletionJob: Job {
    
    // Type to know which Job to return in job creator
    static let type = "FolderDeletionJob"
    // Param
    ...
    func onRun(callback: JobResult) {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + App.Constants.jobDelay) { // Change `2.0` to the desired number of seconds.
            ...
            self.folderLD.deleteFolder(param: self.param)
...
extension FolderDeletionJob: NetworkDelegate {
